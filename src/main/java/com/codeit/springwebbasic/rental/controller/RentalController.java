...
public class RentalController {
    ...
    // 도서 대여
    @PostMapping
    public ResponseEntity<RentalResponseDto> rent(@Valid @RequestBody RentalRequestDto requestDto){
        Rental rental = rentalService.rentBook(requestDto.getMemberId(), requestDto.getBookId());

        return new ResponseEntity<>(RentalResponseDto.from(rental), HttpStatus.CREATED);
    }
